...
class TooLongSegmentCheck(SegmentCheck):
    def __init__(self, max_distance: float = 50.0, units_scale: float = 1.0, verbose: bool = False, logger=None):
    ...
    def run(self, entity, points, output_msp):
        for i in range(len(points) - 1):
            ...
            if self.verbose and self.logger:
                self.logger.log_verbose(f"  Segment {i + 1}: {distance:.3f}m")

            if distance > self.max_distance:
                self.error_count += 1
                if self.verbose and self.logger:
                    self.logger.log_verbose(f"  *** ERROR: Segment exceeds {self.max_distance}m! ***")
